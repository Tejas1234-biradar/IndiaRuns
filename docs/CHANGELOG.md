...
---
## [2026-06-16] — extract JD query payload for vectorization
...
### Why
To prepare for Task 1.4 vectorization, we need to extract the `synthetic_ideal_candidate_embedding_string` generated by the LLM in Task 1.1. Using this first-person narrative string rather than a raw list of bullet points solves the asymmetric search problem by ensuring the query payload mimics the semantic structure of the candidate profiles it will be measured against.
