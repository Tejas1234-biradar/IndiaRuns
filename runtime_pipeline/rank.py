...
def load_candidate_contexts(
    ...
    with _open_candidate_file(candidates_path) as fh:
        for line in fh:
            line = line.strip()
            if not line:
                continue
            ...
            contexts[cid] = {
                ...
                "history_descriptions": [
                    ...
                    for job in history
                    if str(job.get("description", "")).strip()
                ],
                "history_companies": [
...
def format_years(years: float) -> str:
    years = float(years)
    if abs(years - round(years)) < 0.15:
        return str(int(round(years)))
    return f"{years:.1f}"


def city_phrase(location: str) -> str | None:
    if not location:
        return None
...
def normalize_title(ctx: dict) -> str | None:
    title = ctx.get("current_title") or ""
    if title:
        return title
...
def infer_domain_profile(ctx: dict) -> tuple[str | None, str | None, str | None]:
    text_parts = [
        ctx.get("headline", ""),
        ctx.get("summary", ""),
        " ".join(ctx.get("skills", [])),
        " ".join(ctx.get("history_titles", [])),
        " ".join(ctx.get("history_descriptions", [])),
    ]
    ...
    has_retrieval = any(k in blob for k in retrieval_keywords)
    has_llm = any(k in blob for k in llm_keywords)
    has_ml = any(k in blob for k in ml_keywords)
    has_data = any(k in blob for k in data_keywords)
    has_cloud = any(k in blob for k in cloud_keywords)
    has_backend = any(k in blob for k in backend_keywords)

    if has_retrieval and has_llm:
        return (
            "building RAG and retrieval systems",
            "strong NLP + retrieval background",
            "previously shipped retrieval systems in production",
        )
    if has_retrieval:
        return (
            "building search, ranking, and retrieval systems",
            "strong search + retrieval background",
            "previously shipped search and ranking systems",
        )
    if has_ml:
        return (
            "applied ML in production",
            "strong applied ML background",
            "has production ML experience",
        )
    if has_data:
        return (
            "building data and ML infrastructure",
            "strong data / ML infrastructure background",
            "has shipped data infrastructure work",
        )
    if has_cloud:
        return (
            "building cloud and platform systems",
            "strong cloud/platform background",
            "has solid platform engineering exposure",
        )
    if has_backend:
        return (
            "building backend systems",
            "strong backend engineering background",
            "has shipped backend systems in production",
        )
    return (None, None, None)


def infer_environment_phrase(ctx: dict) -> str | None:
    company_blob = " ".join(
        [ctx.get("current_company", ""), *ctx.get("history_companies", [])]
    ).lower()
    industry = (ctx.get("current_industry") or "").lower()
    if (
        any(k in company_blob for k in PRODUCT_COMPANY_KEYWORDS)
        or industry in PRODUCT_INDUSTRIES
    ):
        return "at product companies"
    return None


def support_phrase_from_driver(
    row: pd.Series, driver_keys: list[str], ctx: dict, primary: str
) -> str | None:
    city = city_phrase(ctx.get("location", ""))
    _, domain_background, evidence_phrase = infer_domain_profile(ctx)

    if evidence_phrase and any(
        key in {"num_skills_listed", "faiss_distance_to_jd", "num_previous_jobs"}
    ...
    for key in driver_keys:
        if key == "github_activity_score":
            return f"strong recent GitHub activity ({float(row[key]):.0f})"
        if key in {
            ...
            if city:
                return f"strong recent engagement and {city}"
            return "strong recent engagement"
        if key == "max_assessment_score":
            return f"validated assessment strength ({float(row[key]):.0f})"
        if key == "avg_job_duration_months":
            return f"stable tenure ({float(row[key]):.1f} months average)"
        if key == "faiss_distance_to_jd" and "semantic" not in primary:
            return "strong semantic fit to the JD"
        if key == "num_skills_listed" and domain_background:
            return domain_background
        if key == "years_of_experience" and "years" not in primary:
            return f"solid experience depth ({format_years(row[key])} years)"
        if key == "num_previous_jobs":
            return evidence_phrase or f"experience across {int(row[key])} prior roles"

    return city


def concern_phrase_from_drivers(row: pd.Series, driver_keys: list[str]) -> str | None:
    for key in driver_keys:
        if key == "notice_period_days" and int(row[key]) >= 90:
            return f"some concern on notice period ({int(row[key])} days)"
        if key == "days_since_active" and int(row[key]) >= 90:
            return f"some concern on recent inactivity ({int(row[key])} days)"
        if key == "recruiter_response_rate":
            return f"slightly weaker recruiter responsiveness ({clamp_non_negative(row[key]):.0%})"
        if key == "interview_completion_rate":
            return f"some concern on interview follow-through ({clamp_non_negative(row[key]):.0%})"
        if key == "faiss_distance_to_jd":
            return f"semantic fit is not perfect ({float(row[key]):.3f})"
        if key == "github_activity_score":
            return f"lighter recent GitHub activity ({float(row[key]):.0f})"
    return None


def build_contextual_reasoning(
    row: pd.Series, drivers: list[dict], ctx: dict | None
) -> str:
    ctx = ctx or {}
    title = normalize_title(ctx)
    years = format_years(row["years_of_experience"])
    build_phrase, domain_background, evidence_phrase = infer_domain_profile(ctx)
    environment_phrase = infer_environment_phrase(ctx)

    positive_keys = [
        d["feature_key"] for d in drivers if d["impact"] == "Positive Driver"
    ]
    negative_keys = [
        d["feature_key"] for d in drivers if d["impact"] == "Negative Driver"
    ]

    if title and build_phrase:
        primary = f"{title} with {years} years {build_phrase}"
    elif title:
        primary = f"{title} with {years} years"
    elif domain_background:
        primary = f"{years} years with {domain_background.lower()}"
    else:
        primary = f"{years} years of relevant experience"

    if environment_phrase and environment_phrase not in primary:
        primary = f"{primary} {environment_phrase}"

    support = support_phrase_from_driver(row, positive_keys, ctx, primary)
    if not support and evidence_phrase and "num_skills_listed" in positive_keys:
        support = evidence_phrase

    concern = concern_phrase_from_drivers(row, negative_keys)

    if concern and support:
        return f"{primary}; {support}; {concern} but otherwise strong fit."
    if concern:
        return f"{primary}; {concern} but otherwise strong fit."
    if support:
        return f"{primary}; {support}."
    if domain_background:
        return f"{primary}; {domain_background}."
    return f"{primary}; solid overall fit to the role."
...
def attach_reasoning(
    ...
    top_n = df.head(100).copy()
    explainer = RuntimeCandidateExplainer(ranker)
    drivers = explainer.get_top_drivers(top_n)
    contexts = load_candidate_contexts(candidates_path, top_n["candidate_id"].tolist())

    reasonings = []
    for i, (_, row) in enumerate(top_n.iterrows()):
        reasonings.append(
            build_contextual_reasoning(
                row,
                drivers[i],
                contexts.get(row["candidate_id"]),
            )
        )

    enriched = df.copy()
    enriched.loc[enriched.index[: len(reasonings)], "reasoning"] = reasonings
    return enriched
...
def rank_candidates(
    ...
    top = attach_reasoning(top, ranker, Path(candidates_path))
